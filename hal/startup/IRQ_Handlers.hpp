/** 
 * @file        IRQ_Hnadlers.hpp
 * 
 * @namespace   HAL
 * 
 * @brief       IRQ vector table.
 * 
 * @author      toberg
 * 
 * @date        2026/01/14
...
/**
 * @brief Interrupt_Handler class provides the interrupt vector table and default handlers for the STM32H743xx.
*/
class IRQ_Vector
{
    public:
        using ISR = void (*)();
        static constexpr uint32_t SIZOF_EXCEPTIONS {16U};
        static constexpr uint32_t SIZOF_EXTERNALS  {6U};
        static const ISR table_exceptions[];  
        static const ISR table_externals[];    
};

__attribute__((section(".isr_vector")))
const IRQ_Vector::ISR IRQ_Vector::table_exceptions[SIZOF_EXCEPTIONS] =
{
    reinterpret_cast<ISR>(&_estack),//!< Stack pointer defined in linker script
    // Exception handlers
    &Reset_Handler,                  //!< Reset Handler (Entry point after reset, defined in C_Startup.cpp
    &NMI_Handler,                    //!< NMI
...
const IRQ_Vector::ISR IRQ_Vector::table_externals[] =
{
 
    // External interrupt handlers (starting from entry 16 in the vector table)};
